...
export default function OrdersView({ role }: { role: Role }) {
  ...
  const submitOrder = async (e: React.FormEvent) => {
    ...
    const totalAmount = selectedItems.reduce((acc, curr) => acc + (curr.item.sellingPrice * curr.quantity), 0);
    ...
    const printContent = `
      <div dir="rtl" style="font-family: sans-serif; padding: 20px;">
        <div style="text-align: center; margin-bottom: 20px;">
          <h1 style="margin: 0; color: #1e293b;">کۆگای RF</h1>
          <p style="margin: 5px 0; color: #475569;">هەولێر-ڕێگای کەرکووک</p>
          <p style="margin: 5px 0; font-weight: bold;">٠٧٥٠٦١٤٤٨٩٤</p>
        </div>
        <h2 style="text-align: center; border-top: 2px dashed #cbd5e1; padding-top: 15px; margin-top: 15px;">پسوڵەی داواکاری</h2>
        <div style="margin-bottom: 20px; border-bottom: 1px solid #ccc; padding-bottom: 10px;">
          <p><strong>مەندووب:</strong> ${order.repName}</p>
          <p><strong>مارکێت/شوێن:</strong> ${order.marketName} - ${order.location}</p>
          <p><strong>بەروار:</strong> ${format(order.timestamp, 'yyyy-MM-dd HH:mm')}</p>
        </div>
        <table style="width: 100%; border-collapse: collapse; text-align: right;">
          <thead>
            <tr style="background-color: #f3f4f6;">
              <th style="padding: 8px; border: 1px solid #ccc;">ناوی کاڵا</th>
              <th style="padding: 8px; border: 1px solid #ccc;">ژمارە</th>
              <th style="padding: 8px; border: 1px solid #ccc;">نرخی دانە</th>
              <th style="padding: 8px; border: 1px solid #ccc;">کۆی نرخ</th>
            </tr>
          </thead>
          <tbody>
            ${order.items.map(item => `
              <tr>
                ...
                <td style="padding: 8px; border: 1px solid #ccc;">${(item.price * item.quantity).toLocaleString()}</td>
              </tr>
            `).join('')}
          </tbody>
          <tfoot>
            <tr>
              <th colspan="3" style="padding: 8px; border: 1px solid #ccc; text-align: left;">کۆی گشتی:</th>
              <th style="padding: 8px; border: 1px solid #ccc;">${order.totalAmount.toLocaleString()}</th>
            </tr>
          </tfoot>
        </table>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-4 rounded-2xl shadow-sm border border-slate-200">
        <h2 className="text-xl font-bold text-slate-800">بەشی ئۆردەرەکان</h2>
      ...
      {showNewOrder && (role === 'sales_rep' || role === 'admin') && (
              ...
              <div className="border border-slate-200 rounded-2xl p-4 bg-slate-50">
                ...
                <div className="mt-4 pt-4 border-t border-slate-200 flex justify-between items-center">
                  ...
                  <div className="font-bold text-xl text-indigo-600" dir="ltr">
                    {selectedItems.reduce((acc, curr) => acc + (curr.item.sellingPrice * curr.quantity), 0).toLocaleString()}
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          orders.map((order) => (
            <div key={order.id} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
