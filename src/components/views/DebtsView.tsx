...
export default function DebtsView() {
  ...
  useEffect(() => {
    const q = query(collection(db, 'transactions'), where('type', '==', 'debt'));
  ...
  return (
    <div className="space-y-6">
      <section className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <h3 className="text-lg font-bold mb-4 text-slate-800 border-b border-slate-100 pb-3 flex items-center gap-2">پێدانی قەرز بە مارکێت/کەس</h3>
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          <div>
            <label className="block text-sm text-slate-600 mb-1">ناوی شوێن/کەس</label>
            <input
              type="text"
              required
