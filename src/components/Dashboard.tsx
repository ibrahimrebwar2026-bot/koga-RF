...
export default function Dashboard({ role, onLogout }: DashboardProps) {
  ...
  useEffect(() => {
    ...
    const cashvanMenu = [{ id: 'cashvan_sales', label: 'کاشڤان نەقدە', icon: ShoppingCart }];
  const repMenu = [
    { id: 'orders', label: 'ئۆردەرکردنی کاڵا', icon: ShoppingCart },
  ];

  const menu = role === 'admin' ? adminMenu : role === 'warehouse' ? warehouseMenu : role === 'cashvan' ? cashvanMenu : repMenu;
